...
const DashboardChartsComponent: React.FC<DashboardChartsProps> = ({ data, globalData, currentUser, users = [], onDateChange, startDate, endDate }) => {
  ...
  // CHANGE: Default chart type is now 'bar' (Ngang)
  ...
  const chartData = useMemo(() => {
    ...
    const grouped = processData.reduce((acc, curr) => {
      // FIX: Check valid key to prevent "N/A" columns
      const rawKey = (curr as any)[groupKey];
      
      if (!rawKey || rawKey === 'N/A' || (typeof rawKey === 'string' && rawKey.trim() === '')) {
          return acc;
      }
      ...
      // CHANGE: Scale Ad Spend by 1,000,000 (Millions)
    ...
    const sorted = Object.values(grouped).sort((a: any, b: any) => {
      ...
      // For activity, prioritize Ad Spend (scaled) or Calls for sorting
    ...
    const grouped = globalData.reduce((acc, curr) => {
        ...
        if (!acc[key]) {
            acc[key] = {
                id: key,
                name: displayName, 
                // Capture Hierarchy Names for display
                dss: curr.dss,
                sm: curr.smName,
                volume: 0,
                banca: 0,
  ...
  const maxValue = useMemo(() => {
    if (viewType === 'chart') {
        return Math.max(...chartData.map((d: any) => {
          ...
          // For activity, compare calls, flyers, and the scaled AdSpend
  ...
  const maxAppValue = useMemo(() => {
      if (chartType === 'combo') {
          return Math.max(...chartData.map((d: any) => d.appPL), 1);
      }
      return 1;
  }, [chartData, chartType]);
                       ...
                       items.map((item, idx) => {
                           ...
                           return (
                               <div key={idx} className="flex items-center gap-3 mb-3 last:mb-0 p-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg transition-colors border-b border-gray-50 dark:border-gray-700 last:border-0">
                                   ...
                                   <div className="w-16 h-16 rounded-full border-2 border-gray-100 dark:border-gray-600 overflow-hidden flex-shrink-0 bg-gray-50 dark:bg-gray-700 shadow-sm">
                                       {avatar ? (
                                           <img src={avatar} alt="" className="w-full h-full object-cover"/> 
                                       ) : (
                                           <div className={`w-full h-full flex items-center justify-center text-xl font-bold ${rankingScope !== 'dsa' ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-300' : 'bg-gray-100 text-gray-400 dark:bg-gray-600 dark:text-gray-300'}`}>
                                   ...
                                   <div className="flex-1 min-w-0">
                                       <div className="flex justify-between items-center text-xs mb-1">
                                           <div>
                                              <span className={`font-bold block truncate text-sm mb-0.5 ${isTop1 ? 'text-gray-900 dark:text-gray-100' : 'text-gray-700 dark:text-gray-300'}`}>{item.name}</span>
                                              {rankingScope === 'dsa' && item.dss && (
                                                <span className="text-[10px] italic text-gray-500 dark:text-gray-400 block truncate" title="Quản lý trực tiếp">
                                                  {item.dss}
                                                </span>
                                              )}
                                              {rankingScope === 'dss' && item.sm && (
                                                <span className="text-[10px] italic text-gray-500 dark:text-gray-400 block truncate" title="Thuộc SM">
                                                  {item.sm}
                                                </span>
                                              )}
                                           </div>
                                           <span className={`font-bold font-mono text-right text-sm ${style.text}`}>
        ...
        {chartData.map((item: any) => (
            <div key={item.key} className="flex flex-col items-center group relative min-w-[60px]">
                <div className="flex items-end space-x-1 h-[300px] relative">
                    {/* Bar 1 */}
                    <div 
                        className={`w-8 md:w-10 rounded-t-lg transition-all duration-500 relative shadow-sm ${activeTab === 'financial' ? 'bg-emerald-500' : activeTab === 'apps' ? 'bg-indigo-500' : 'bg-orange-500'}`}
                        style={{ height: `${Math.max(((activeTab === 'financial' ? item.totalVolume : activeTab === 'apps' ? item.appPL : item.calls) / maxValue) * 100, 2)}%` }}
                    >
                    ...
                    {/* Bar 2 */}
                    <div 
                        className={`w-8 md:w-10 rounded-t-lg transition-all duration-500 relative shadow-sm ${activeTab === 'financial' ? 'bg-blue-500' : activeTab === 'apps' ? 'bg-rose-500' : 'bg-amber-400'}`}
                        style={{ height: `${Math.max(((activeTab === 'financial' ? item.totalBanca : activeTab === 'apps' ? item.appCRC : item.flyers) / maxValue) * 100, 2)}%` }}
                    >
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 text-[10px] md:text-xs font-bold text-gray-700 dark:text-gray-200 whitespace-nowrap z-20">
                    ...
                    {/* Bar 3 (Apps: Loan CRC, Activity: AdSpend/DLK) */}
                    {(activeTab === 'apps' || activeTab === 'activity') && (
                        <div 
                            className={`w-8 md:w-10 rounded-t-lg transition-all duration-500 relative shadow-sm ${activeTab === 'apps' ? 'bg-emerald-500' : 'bg-red-500'}`}
                            style={{ height: `${Math.max(((activeTab === 'apps' ? item.loanCRC : item.adSpend) / maxValue) * 100, 2)}%` }}
                        >
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 text-[10px] md:text-xs font-bold text-gray-700 dark:text-gray-200 whitespace-nowrap z-20">
                                {formatValue(activeTab === 'apps' ? item.loanCRC : item.adSpend)}
                                {activeTab === 'activity' && 'Tr'}
                            </div>
                        </div>
                    )}
                </div>
                <div className="mt-4 text-xs font-bold text-gray-600 dark:text-gray-300 rotate-0 md:-rotate-45 origin-top-left md:translate-y-3 w-28 truncate text-center md:text-left cursor-default" title={item.key}>
                    {item.key}
                </div>
            </div>
        ))}
    ...
    <div className="flex flex-col space-y-4 w-full h-full overflow-y-auto px-2">
        {chartData.map((item: any) => (
            <div key={item.key} className="flex items-start w-full border-b border-gray-50 dark:border-gray-700 pb-3 last:border-0">
                <div className="w-32 text-xs font-bold text-gray-700 dark:text-gray-300 truncate mr-3 text-right pt-1" title={item.key}>
                    {item.key}
                </div>
                
                {/* DEFAULT BARS FOR FINANCIAL & APPS */}
                {activeTab !== 'activity' && (
                  <div className="flex-1 flex flex-col space-y-1.5">
                      {/* Bar 1 */}
                      <div className="flex items-center">
                          <div 
                              className={`h-5 rounded-r-md transition-all duration-500 ${activeTab === 'financial' ? 'bg-emerald-500' : 'bg-indigo-500'}`}
                              style={{ width: `${Math.max(((activeTab === 'financial' ? item.totalVolume : item.appPL) / maxValue) * 100, 2)}%` }}
                          ></div>
                          <span className="ml-2 text-xs font-bold text-gray-600 dark:text-gray-400">
                              {formatValue(activeTab === 'financial' ? item.totalVolume : item.appPL)}
                          </span>
                      </div>
                      {/* Bar 2 */}
                      <div className="flex items-center">
                          <div 
                              className={`h-5 rounded-r-md transition-all duration-500 ${activeTab === 'financial' ? 'bg-blue-500' : 'bg-rose-500'}`}
                              style={{ width: `${Math.max(((activeTab === 'financial' ? item.totalBanca : item.appCRC) / maxValue) * 100, 2)}%` }}
                          ></div>
                          <span className="ml-2 text-xs font-bold text-gray-600 dark:text-gray-400">
                              {formatValue(activeTab === 'financial' ? item.totalBanca : item.appCRC)}
                          </span>
                      </div>
                      {/* Bar 3 (Apps) */}
                      {activeTab === 'apps' && (
                          <div className="flex items-center">
                              <div 
                                  className="h-5 rounded-r-md transition-all duration-500 bg-emerald-500"
                                  style={{ width: `${Math.max((item.loanCRC / maxValue) * 100, 2)}%` }}
                              ></div>
                              <span className="ml-2 text-xs font-bold text-gray-600 dark:text-gray-400">
                                  {formatValue(item.loanCRC)}
                              </span>
                          </div>
                      )}
                  </div>
                ...
                {/* EXPANDED BAR CHART FOR ACTIVITY TAB */}
                {activeTab === 'activity' && (
                   <div className="flex-1 flex flex-col space-y-1.5">
                       {/* Bar 1: Calls */}
                       <div className="flex items-center">
                           <div 
                               className="h-4 rounded-r-md transition-all duration-500 bg-orange-500"
                               style={{ width: `${Math.max((item.calls / maxValue) * 100, 2)}%` }}
                           ></div>
                           <span className="ml-2 text-[10px] font-bold text-gray-500 dark:text-gray-400">
                               {item.calls} Calls
                           </span>
                       </div>
                       ...
                       <div className="flex items-center">
                           <div 
                               className="h-4 rounded-r-md transition-all duration-500 bg-amber-400"
                               style={{ width: `${Math.max((item.flyers / maxValue) * 100, 2)}%` }}
                           ></div>
                           <span className="ml-2 text-[10px] font-bold text-gray-500 dark:text-gray-400">
                               {formatValue(item.flyers)} Tờ rơi
                           </span>
                       </div>

                       {/* Bar 3: Ad Spend (Scaled by Million) */}
                       <div className="flex items-center">
                           <div 
                               className="h-4 rounded-r-md transition-all duration-500 bg-red-500"
                               style={{ width: `${Math.max((item.adSpend / maxValue) * 100, 2)}%` }}
                           ></div>
                           <span className="ml-2 text-[10px] font-bold text-red-600 dark:text-red-400">
                               {formatValue(item.adSpend)} Tr QC
                           </span>
                       </div>
  ...
  const renderPieChart = () => {
      // Calculate share of the PRIMARY metric
      ...
      const conicStyle = { background: `conic-gradient(${gradientParts.join(', ')})` };

      return (
          <div className="h-full flex flex-col md:flex-row items-center justify-center gap-8">
              <div className="relative w-64 h-64 rounded-full shadow-lg border-4 border-white dark:border-gray-700" style={conicStyle}>
  ...
  const renderComboChart = () => {
    // Fixed layout for SVG alignment
    const itemWidth = 80; // Width of each bar container
    const gap = 32; // Margin right
    const chartHeight = 300;
    const totalWidth = chartData.length * (itemWidth + gap);

    // Calculate SVG Points for the Line (Apps)
    const points = chartData.map((d: any, i: number) => {
        const x = i * (itemWidth + gap) + (itemWidth / 2); // Center of the bar
        const y = chartHeight - (d.appPL / maxAppValue) * chartHeight;
        return `${x},${y}`;
    }).join(' ');

    return (
        <div className="relative h-full" style={{ width: `${totalWidth}px` }}>
            {/* SVG Layer for Line Chart */}
            <svg className="absolute top-0 left-0 w-full h-[300px] z-10 pointer-events-none overflow-visible">
                ...
                {chartData.map((d: any, i: number) => {
                    const x = i * (itemWidth + gap) + (itemWidth / 2);
                    const y = chartHeight - (d.appPL / maxAppValue) * chartHeight;
                    return (
                        <circle key={i} cx={x} cy={y} r="5" fill="white" stroke="#f97316" strokeWidth="3" />
                    );
                })}
            </svg>
                ...
                {chartData.map((item: any) => (
                    <div key={item.key} className="flex flex-col items-center group relative" style={{ width: `${itemWidth}px`, marginRight: `${gap}px` }}>
                        <div className="flex items-end justify-center w-full h-[300px] relative">
                            {/* Bar (Volume) */}
                            <div 
                                className="w-12 rounded-t-lg transition-all duration-500 relative shadow-sm bg-emerald-500/90 hover:bg-emerald-500"
                                style={{ height: `${Math.max((item.totalVolume / maxValue) * 100, 2)}%` }}
                            >
                                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 text-[10px] md:text-xs font-bold text-gray-700 dark:text-gray-200 whitespace-nowrap z-20">
                                    {formatValue(item.totalVolume)}
                                </div>
                            </div>
                            
                            {/* Point Label (Apps) - Absolute positioned based on height */}
                            <div 
                                className="absolute left-1/2 -translate-x-1/2 bg-orange-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full z-20 shadow-sm"
                                style={{ bottom: `${(item.appPL / maxAppValue) * 100}%`, marginBottom: '10px' }}
                            >
                                {item.appPL} App
                            </div>
                        </div>
                        <div className="mt-4 text-xs font-bold text-gray-600 dark:text-gray-300 w-full truncate text-center cursor-default" title={item.key}>
                            {item.key}
                        </div>
                    </div>
                ))}
    ...
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 h-full flex flex-col transition-colors duration-300">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-6 gap-4">
        ...
        <div className="flex flex-col md:flex-row items-center gap-3 w-full xl:w-auto">
          ...
          {/* Chart Type Dropdown (Only visible in Chart View) */}
          {viewType === 'chart' && (
              <div className="relative">
                  <select 
                    value={chartType}
                    onChange={(e) => setChartType(e.target.value as ChartType)}
                    className="appearance-none bg-gray-100 dark:bg-gray-700 border-none rounded-lg py-2 pl-9 pr-8 text-sm font-bold text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-emerald-500 cursor-pointer"
                  >
          ...
          <div className="bg-gray-100 dark:bg-gray-700 p-1 rounded-lg flex shadow-inner">
             <button onClick={() => setViewType('chart')} className={`px-4 py-2 rounded-md text-sm font-bold flex items-center transition-all ${viewType === 'chart' ? 'bg-white dark:bg-gray-600 text-emerald-700 dark:text-emerald-300 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
                <LayoutGrid size={16} className="mr-2"/> Biểu Đồ
             </button>
      ...
      {viewType === 'chart' && (
        <div className="flex justify-center space-x-6 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700 text-sm text-gray-600 dark:text-gray-300 flex-wrap">
            {chartType !== 'combo' && (
              <>
                <div className="flex items-center">
                    <div className={`w-3 h-3 rounded-full mr-2 ${activeTab === 'financial' ? 'bg-emerald-500' : activeTab === 'apps' ? 'bg-indigo-500' : 'bg-orange-500'}`}></div>
                    <span className="font-medium">
                        {activeTab === 'financial' ? 'Volume (Direct + FEOL)' : activeTab === 'apps' ? 'App PL (Tổng)' : 'Calls (Cuộc gọi)'}
                    </span>
                </div>
                {chartType !== 'pie' && (
                    <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${activeTab === 'financial' ? 'bg-blue-500' : activeTab === 'apps' ? 'bg-rose-500' : 'bg-amber-400'}`}></div>
                        <span className="font-medium">
                            {activeTab === 'financial' ? 'Banca' : activeTab === 'apps' ? 'App CRC (Thẻ)' : 'Flyers (Tờ rơi)'}
                        </span>
                    </div>
                )}
                {/* Apps Tab Extra Legend */}
                {activeTab === 'apps' && chartType !== 'pie' && (
                    <div className="flex items-center">
                        <div className="w-3 h-3 rounded-full mr-2 bg-emerald-500"></div>
                        <span className="font-medium">Loan CRC (Đã cấp)</span>
                    </div>
                )}
                {/* Activity Tab Extra Legend */}
                {activeTab === 'activity' && chartType !== 'pie' && (
                    <div className="flex items-center space-x-4">
                        <div className="flex items-center">
                           <div className="w-3 h-3 rounded-full mr-2 bg-red-500"></div>
                           <span className="font-medium">Chi phí QC (x1.000.000đ)</span>
                        </div>
                        <div className="flex items-center">
                           <div className="w-3 h-3 rounded-full mr-2 bg-blue-500"></div>
                           <span className="font-medium">CTV</span>
                        </div>
                        <div className="flex items-center">
                           <div className="w-3 h-3 rounded-full mr-2 bg-purple-500"></div>
                           <span className="font-medium">ĐLK</span>
                        </div>
                    </div>
                )}
              </>
            ...
            {/* Legend for Combo Chart */}
            {chartType === 'combo' && (
              <>
                 <div className="flex items-center">
                    <div className="w-3 h-3 rounded-full mr-2 bg-emerald-500"></div>
                    <span className="font-medium">Doanh Số (Cột)</span>
                </div>
                 <div className="flex items-center">
                    <div className="w-8 h-1 mr-2 bg-orange-500 flex items-center justify-center relative">
                        <div className="w-2 h-2 rounded-full bg-white border-2 border-orange-500"></div>
                    </div>
                    <span className="font-medium">Tổng App (Đường)</span>
                </div>
              </>
            )}
        </div>
